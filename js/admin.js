...
// -------------------- EXPORT DATA.JS FOR GITHUB PAGES --------------------
function exportDataJs() {
  const products = getStoredProducts();
  const categories = getStoredCategories();
  const config = (typeof STORE_CONFIG !== 'undefined') ? STORE_CONFIG : {};

  const timestamp = new Date().toISOString();
  const versionTag = "v_" + Date.now();

  const fileHeader = `/**\n * Dream Cart BD - Global Data Source for GitHub Pages\n * Generated by Admin Panel\n * Date: ${new Date().toLocaleString('en-US')}\n */\n\n`;
  const productsCode = `const INITIAL_PRODUCTS = ${JSON.stringify(products, null, 2)};\n\n`;
  const categoriesCode = `const INITIAL_CATEGORIES = ${JSON.stringify(categories, null, 2)};\n\n`;
  const configCode = `const STORE_CONFIG = ${JSON.stringify(config, null, 2)};\n\n`;

  const syncCode = `// Database Initializer (Sync with localStorage & GitHub Updates)\n` +
                   `const DCB_DATA_VERSION = "${versionTag}";\n\n` +
  ...
  const fullContent = fileHeader + productsCode + categoriesCode + configCode + syncCode;
  ...
  showAdminNotification('✅ data.js ডাউনলোড হয়েছে! এটি GitHub-এর js ফোল্ডারে রিপ্লেস করে Commit দিন। সকল কাস্টমার সাথে সাথে আপডেট দেখতে পাবে!', 'success');
}
